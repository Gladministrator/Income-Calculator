...
namespace Pay_Application
{
    public partial class Index : System.Web.UI.Page
    {
        ...
        private void Compensation_Calcs (TextBox hours, TextBox payRate)
        {
            ...
            decimal dRegularPay;
            decimal dTaxes;
            var dTotalHours = Convert.ToDecimal(hours.Text);
            var dPayRate = Convert.ToDecimal(payRate.Text);
            if (dTotalHours > 40M)
            {
                var dOvertimeHours = dTotalHours - 40M;
                PayOvertimeHoursText.Text = dOvertimeHours.ToString("F");

                var dOvertimePay = dOvertimeHours * dPayRate * 1.5M;
        ...
        private bool Validate_Number (TextBox numberData, Label infoLabelTip, decimal limit = decimal.MaxValue)
        {
            try
            {
                var dNumber = Convert.ToDecimal(numberData.Text);
